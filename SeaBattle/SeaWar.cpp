...
//методы, необходимые дл€ настроек игры, вызываютс€ при инициализации
void SeaWar::mode_selection_fill() {
	double s;
	std::cout << "¬ыберите режим расставлени€ кораблей(1-рандомно, 0-заполн€ет игрок): ";
	while (!(std::cin >> s) || !(s == 1 || s == 0)) {
		std::cin.clear();
		std::cin.ignore();
		system("cls");
		std::cout << "¬ы допустили ошибку!" << std::endl;
		std::cout << "¬ыберите режим расставлени€ кораблей(1-рандомно, 0-заполн€ет игрок): ";
	}
	if (s == 1)
...
void SeaWar::first_move() {
	double s;
	std::cout << "¬ы ходите первым? (1-да, 0-нет): ";
	while (!(std::cin >> s) || !(s == 1 || s == 0)) {
		std::cin.clear();
		std::cin.ignore();
		system("cls");
		std::cout << "¬ы допустили ошибку!std::" << std::endl;
		std::cout << "¬ы ходите первым? (1-да, 0-нет): ";
	}
	if (s == 1)
		player_1_move = true;
...
//методы дл€ правильного вывода на экран
...
//методы дл€ заполнени€ пол€ корабл€ми
...
void SeaWar::fill_ship(bool random, int m[10][10], int cnt_ship[5], char f[12][12]) {
	...
	if (random) {
		...
		std::string way;
		std::cout << "¬ы выбрали самосто€тельное заполнение пол€" << std::endl;
		while (!correct_cnt_ship(cnt_ship)) {
			std::cout << "ƒанные корабл€(x, y, size, course(e, w, s или n): ";
			...
			while (!(std::cin >> x00) ||
				...
				system("cls");
				print_field(f);
				std::cout << "¬ы совершили ошибку!!!" << std::endl;
				std::cout << "ƒанные корабл€(x, y, size, course(e, w, s или n): ";
			}
			x0 = x00;
			y0 = y00;
		...
		std::cout << std::endl;
		std::cout << "ѕоле заполнено";
	}
}
//методы дл€ механики бо€
...
void SeaWar::next_move() {
	...
	//если a == 110, то координаты дл€ бота создаютс€ новые, но с другими услови€ми
	while (a == 110) {
		k++;
		a = get_coord(player_1_move);

		//защита от бесконечного цикла, дл€ отладки было полезно, когда находил баги
		if (k == 200) {
			continue;
			//system("pause");
			//break;
